...
export class MySQL implements Database {
        ...
        if (!transaction) {
            ...
            return this.prepareTransaction(transaction)
                .then((transaction) => new Promise<T>((resolve, reject) => {
                    const connection: Connection = transaction.connection as Connection;
                    connection.query(query, data, (err, result) => {
                        if (err && err.fatal) {
                            reject(err);
                        } else if (err) {
    ...
    private updateOne<T>(model: string, value: T, transaction?: Transaction): Promise<IResponse<T>> {
        ...
        return prepare.then((transaction) => {
            ...
            for (const key in analysedValue.lists) {
                if (analysedValue.lists.hasOwnProperty(key)) {
                    steps.push(this.updateList(new this.models[model]({ id }), key, analysedValue.lists[key], transaction));
                }
            }
        ...
        if (query.fields && query.fields.length) {
            for (let i = 0; i < query.fields.length; i++) {
                if (query.fields[i] instanceof Vql) {
                    fields.push(this.getSubQuery(query.fields[i] as Vql));
                } else if (modelFields[query.fields[i] as string]) {
                    if (modelFields[query.fields[i] as string].properties.type == FieldType.List) { continue; }
    ...
    private getCondition(model: string, condition: Condition, alias: string = model) {
        ...
        if (!condition.isConnector) {
            ...
            for (let i = 0; i < condition.children.length; i++) {
                const childCondition = this.getCondition(model, condition.children[i]).trim();
                if (childCondition) {
